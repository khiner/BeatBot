...
public class Vertices {
	...
	public Vertices(GL10 gl, int maxVertices, int maxIndices, boolean hasColor,
			boolean hasTexCoords, boolean hasNormals) {
		this(gl, maxVertices, maxIndices, hasColor, hasTexCoords, hasNormals,
				false); // Call Overloaded Constructor
	}
	...
	// R: [none]
	public void bind() {
		gl.glEnableClientState(GL10.GL_VERTEX_ARRAY); // Enable Position in
														// Vertices
		vertices.position(0); // Set Vertex Buffer to Position
		gl.glVertexPointer(positionCnt, GL10.GL_FLOAT, vertexSize, vertices); // Set
																				// Vertex
																				// Pointer

		if (hasColor) { // IF Vertices Have Color
			gl.glEnableClientState(GL10.GL_COLOR_ARRAY); // Enable Color in
															// Vertices
			vertices.position(positionCnt); // Set Vertex Buffer to Color
			gl.glColorPointer(COLOR_CNT, GL10.GL_FLOAT, vertexSize, vertices); // Set
																				// Color
																				// Pointer
		}
		...
		if (hasNormals) {
			gl.glEnableClientState(GL10.GL_NORMAL_ARRAY); // Enable Normals in
															// Vertices
			vertices.position(positionCnt + (hasColor ? COLOR_CNT : 0)
					+ (hasTexCoords ? TEXCOORD_CNT : 0)); // Set Vertex Buffer
															...
															// is also
															// specified)
			gl.glNormalPointer(GL10.GL_FLOAT, vertexSize, vertices); // Set
																		// Normals
																		// Pointer
		}
	}
	...
	public void draw(int primitiveType, int offset, int numVertices) {
		if (indices != null) { // IF Indices Exist
			...
			gl.glDrawArrays(primitiveType, offset, numVertices); // Draw Direct
																	// (Array)
		}
	}
	...
	public void unbind() {
		...
		if (hasTexCoords) // IF Vertices Have Texture Coords
			gl.glDisableClientState(GL10.GL_TEXTURE_COORD_ARRAY); // Clear
																	// Texture
																	// Coords
																	// State

		if (hasNormals) // IF Vertices Have Normals
			gl.glDisableClientState(GL10.GL_NORMAL_ARRAY); // Clear Normals
															// State
	}
