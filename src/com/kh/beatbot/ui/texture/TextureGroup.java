...
public class TextureGroup extends MeshGroup {
	...
	private synchronized void textureVertices(Mesh mesh, int textureVertex,
		...
		vertex(mesh, textureVertex + 1, x + width, y + height, color,
				region.u2, region.v2);
		vertex(mesh, textureVertex + 2, x + width, y, color, region.u2,
				region.v1);
	...
	private synchronized void vertex(Mesh mesh, int index, float x, float y,
			float[] color, float textureX, float textureY) {
		vertex(mesh, index, x, y, color);
		textureVertex(mesh, index, textureX, textureY);
	}

	private synchronized void textureVertex(Mesh mesh, int index,
