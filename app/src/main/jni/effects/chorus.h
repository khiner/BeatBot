...
static inline void chorus_process(ChorusConfig *config, float **buffers, int size) {
    ...
    for (samp = 0; samp < size; samp++) {
        float dTimeL = config->baseTime * 0.707f
                       * (1.0f + config->modAmt * sinewave_valueAtOffset(config->mod[0], samp));
        float dTimeR = config->baseTime * 0.5f
                       * (1.0f - config->modAmt * sinewave_valueAtOffset(config->mod[1], samp));
